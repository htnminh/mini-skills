...
| [orchestrate-lite](./skills/orchestrate-lite/SKILL.md) | Explores the code, plans, delegates the implementation to one sub-agent, then verifies and fixes the result itself. | You want a code change done under your own plan and review, without a full agent team. |
...
| [tldr-session](./skills/tldr-session/SKILL.md) | Answers a question or summarizes the previous response in under 30 words, without tool calls, every turn until told to stop. | You want short replies for the rest of the session instead of one-off. |
| [update-mini-skills](./skills/update-mini-skills/SKILL.md) | Fetches the installer from the Mini Skills repository and follows it. | You want to install the collection or update it to the latest upstream version. |
